...
Describe, in plain language, how a `PATCH_DIAZOTROPH` workorder is executed and how a `yield_bundle` is produced.
...
4. **Handle budget exhaustion before finalization**
   - If the elapsed time already exceeds the budget:
     ...
     - emit a `yield_bundle` with an empty patch and explanatory notes.
...
5. **Attempt to apply the patch**
   ...
   - If apply fails:
     ...
     - emit a `yield_bundle` containing the attempted patch and failure notes.
...
7. **Return terminal success artifact**
   ...
   - Emit a `yield_bundle` containing:
     - ticket id,
     - branch name,
     - normalized patch,
     - summary notes,
...
- One runner invocation emits exactly one `yield_bundle`.
