...
5. **Run the assigned diazotroph**
   ...
   - Persist exactly one `yield_bundle` from runner output.

6. **Evaluate gates and record the run**
   - Evaluate gates against the ticket and persisted yield.
...
- One executed workorder produces one persisted `yield_bundle` and one `run_record`.
