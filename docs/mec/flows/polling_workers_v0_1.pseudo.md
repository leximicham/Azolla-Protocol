...
v0.1 workers maintain claim metadata in implementation storage (for example, a lock table keyed by `event_id` or `work_order_id`) and must validate each record against `docs/mec/schemas/claim_record.schema.json`.
